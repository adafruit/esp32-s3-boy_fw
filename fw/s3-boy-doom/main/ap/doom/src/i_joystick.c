...
// TODO chcbaram
    ...
    joybstraferight = -1;
    joybprevweapon = -1;
    joybnextweapon = _BTN_START;
    joybmenu    = _BTN_SELECT;
    joybautomap = _BTN_Y;
#endif
}
...
static int GetAxisState(int axis, int invert)
{
  ...
  if (axis == 0)
  {
    ...
    if (buttonGetPressed(_BTN_LEFT))
    {
      joy_x_out = -(32767/4 + DEAD_ZONE);
    }
    if (buttonGetPressed(_BTN_RIGHT))
    {
      joy_x_out = +(32767/4 + DEAD_ZONE);
    }
    result = joy_x_out;
  ...
  if (axis == 1)
  {
    ...
    if (buttonGetPressed(_BTN_DOWN))
    {
      joy_y_out = -(32767/4 + DEAD_ZONE);
    }
    if (buttonGetPressed(_BTN_UP))
    {
      joy_y_out = +(32767/4 + DEAD_ZONE);
    }

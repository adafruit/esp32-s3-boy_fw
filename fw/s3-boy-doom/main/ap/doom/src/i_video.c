...
EXT_RAM_BSS_ATTR uint8_t fullscreen_buffer[HW_LCD_WIDTH * HW_LCD_HEIGHT];
...
void I_SetPalette (byte *doompalette)
{
    ...
    for (i = 0; i < 256; i++)
    {
      ...
      pixel_color = GFX_RGB565(gammatable[usegamma][c->r],
                       gammatable[usegamma][c->g],
                       gammatable[usegamma][c->b]);

      rgb565_palette[i]  = (pixel_color>>8) & 0x00FF;
      rgb565_palette[i] |= (pixel_color<<8) & 0xFF00;
      doompalette += 3;
    }
#endif
}
